...
namespace nu::json
{
	...
	bool Read(const rapidjson::Value& value, const std::string& name, Vector2& data, bool required = false);
	bool Read(const rapidjson::Value& value, const std::string& name, Vector3& data, bool required = false);
}